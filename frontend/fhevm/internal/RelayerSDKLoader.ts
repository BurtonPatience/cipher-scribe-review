...
import { SDK_CDN_URLS, SDK_CDN_URL } from "./constants";
...
export class RelayerSDKLoader {
  ...
  public load(): Promise<void> {
    ...
    // Try to load SDK from multiple URLs with fallback
    return this._loadWithFallback(0);
  }

  private _loadWithFallback(urlIndex: number): Promise<void> {
    if (urlIndex >= SDK_CDN_URLS.length) {
      throw new Error(`RelayerSDKLoader: Failed to load SDK from all ${SDK_CDN_URLS.length} URLs`);
    }

    const currentUrl = SDK_CDN_URLS[urlIndex];
    console.log(`[RelayerSDKLoader] trying URL ${urlIndex + 1}/${SDK_CDN_URLS.length}: ${currentUrl}`);
    ...
    return new Promise((resolve, reject) => {
      ...
      script.onload = () => {
        console.log("[RelayerSDKLoader] script loaded successfully from:", currentUrl);
        console.log("[RelayerSDKLoader] checking window.relayerSDK:", !!(window as any).relayerSDK);

        if (!isFhevmWindowType(window, this._trace)) {
          console.log("[RelayerSDKLoader] script onload FAILED...");
          console.log("[RelayerSDKLoader] window object:", window);
          // Try next URL
          console.log("[RelayerSDKLoader] trying next URL...");
          this._loadWithFallback(urlIndex + 1).then(resolve).catch(reject);
        } else {
      ...
      script.onerror = (event) => {
        console.log("[RelayerSDKLoader] script onerror for URL:", currentUrl, event);
        // Try next URL
        console.log("[RelayerSDKLoader] trying next URL...");
        this._loadWithFallback(urlIndex + 1).then(resolve).catch(reject);
      };
